...
public class MindTestUI : MonoBehaviour {
    ...
    public void StartTest() {
        if (firstConnectStatus_Text.text == "Connected") {
            if (closeFirstUI_Bool == false) {
                ...
                saveCom_String = mindUI_GameObject[0].GetComponent<MindLink>().comPort_InputField.text;
                mindGear.OnApplicationQuit();
                mindUI_GameObject[1].SetActive(true);
                mindUI_GameObject[0].SetActive(false);
                mindUI_GameObject[1].GetComponent<MindLink>().comPort_InputField.text = saveCom_String;
    ...
    public void FirstTest() {
        if (mindUI_GameObject[1].GetComponent<MindTest>().mindUINext_Bool == true) {
            ...
            mindUI_GameObject[2].GetComponent<MindLink>().comPort_InputField.text = saveCom_String;
    ...
    public void SecondTest() {
        if (mindUI_GameObject[2].GetComponent<MindTest>().mindUINext_Bool == true) {
            PlayerPrefs.SetString("gamecom", saveCom_String);
            PlayerPrefs.SetString("gameuser", AccountSetting.nowUser);
            PlayerPrefs.SetString("gameLS", PlayerPrefs.GetString("getLS"));
    ...
    public IEnumerator LevelSave() {
        ...
        data.Add("getuser", AccountSetting.nowUser);
    ...
    IEnumerator LoadLevelWithBar(string scene) {
        _async = Application.LoadLevelAsync(scene);
        while (!_async.isDone) {
